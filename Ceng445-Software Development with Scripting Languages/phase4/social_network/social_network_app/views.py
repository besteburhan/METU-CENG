...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if self.user_form.is_valid():
            ...
            Profile.objects.create(user=user, verification_code=generated_password)
            send_mail(
            ...
            if old_pass != "":
                if usr.check_password(old_pass):
                    ...
                    messages.warning(request,
                                     "Wrong Password, if you want to get new password, please do not fill old password field.")
                ...
                usr.save()
                send_mail(
                    'Social Network Change Password',
        ...
        if user_id:
            if request.user.id != user_id:
                ...
                try:
                    ...
                    pass
                if friend_state == 0:
                    location_available_items = Item.objects.filter(Q(owner=user_id) & Q(view=3) & ~Q(id__in=br)).distinct()
                    location_unavailable_items = Item.objects.filter(Q(owner=user_id) & Q(view=3) & Q(id__in=br)).distinct()
                else:
                    location_available_items = Item.objects.filter(Q(owner=user_id) & Q(view__gte=friend_state) & ~Q(id__in=br)).distinct()
                    location_unavailable_items = Item.objects.filter(Q(owner=user_id) & Q(view__gte=friend_state) & Q(id__in=br)).distinct()
            else:  # own items

                location_available_items = Item.objects.filter(Q(owner=request.user)).distinct()
    ...
    def post(self, request, user_id):
        ...
        if user_id == request.user.id:
            if request.POST['submit_type'] == 'save':
                ...
                watch_requests = WatchRequest.objects.filter(Q(followed_user=request.user) & Q(watch_method=2)).distinct()
                ...
                borrow_requests = WatchRequest.objects.filter(Q(item=borrow_item.item) & Q(watch_method=1)).distinct()
...
class FriendsView(View):
    def get(self, request):
        friends = {}
        if request.user:
            friends = Friend.objects.filter(Q(sender_user=request.user) & ~Q(state=0)).distinct()
            friends = friends | Friend.objects.filter(Q(receiver_user=request.user) & ~Q(state=0)).distinct()

        friend_requests = {}
        if request.user:
            friend_requests = Friend.objects.filter(Q(receiver_user=request.user) & Q(state=0)).distinct()
...
class BorrowRequestsView(View):
    def get(self, request):
        borrow_requests = {}
        if request.user:
            borrow_requests = BorrowRequest.objects.filter(Q(user=request.user) & ~Q(item__owner=request.user) ).order_by('request_date').distinct()
...
class AllItemsView(View):
        ...
        viewable_items = Item.objects.filter(~Q(view=0) & ~Q(owner=request.user) &
                                               ( Q(view=3) |
                                                (Q(owner_id__in=s_users) & ( ( Q(owner__friend_sender_user__state=2 ) & Q(view=2)) | ( Q(owner__friend_sender_user__state=1 ) & Q(view__gte=1))) )
                                                 | ( Q(owner_id__in=r_users) &( ( Q(owner__friend_receiver_user__state=2 ) & Q(view=2)) | ( Q(owner__friend_receiver_user__state=1 ) & Q(view__gte=1)))))).distinct()
        # borrowable
        ...
        borrowable_items = Item.objects.filter(~Q(id__in=already_requested) & ~Q(owner=request.user) & ~Q(borrow=0) &
                                               (Q(borrow=3) |
                                                ...
                                                | (Q(owner_id__in=r_users) & ((Q(owner__friend_receiver_user__state=2) &
                                                                               Q(borrow=2)) | (Q(owner__friend_receiver_user__state=1) & Q(borrow__gte=1)))))).distinct()
    ...
    def post(self, request):
        if request.POST['submit_type'] == 'search':
            ...
            s_friends = Friend.objects.filter(Q(receiver_user=request.user)).values('sender_user', 'state').distinct()
            r_friends = Friend.objects.filter(Q(sender_user=request.user)).values('receiver_user', 'state').distinct()
            viewable_items = {}
            borrowable_items = {}
            receivers = []
            receiver_states = []
                ...
                borrowable_items = Item.objects.filter(~Q(owner=request.user) &
                    ~Q(borrow=0) & ( Q(borrow=3) | ( (Q(borrow=1) & ( Q(owner_id__in=senders) & Q(owner__friend_sender_user__state=1))) |
                                     ...
                                     ((Q(borrow=1) & ( Q(owner_id__in=receivers) & Q(owner__friend_receiver_user__state=1))) |
                                      ( Q(borrow=2) & (Q(owner_id__in=receivers) & Q(owner__friend_receiver_user__state__gte=1)))))).distinct()

                for word in words:
                    borrowable_items = borrowable_items.filter(Q(artist__contains=word) | Q(title__contains=word)).distinct()
                    if genre != '':
                ...
                viewable_items = Item.objects.filter(~Q(owner=request.user) & ~Q(view=0) & ( Q(view=3) | ( (Q(view=1) & (
                        Q(owner_id__in=senders) & Q(owner__friend_sender_user__state=1))) | ( Q(view=2) & ( Q(owner_id__in=senders) & Q(owner__friend_sender_user__state__gte=1)))) |
                                         (( Q(view=1) & ( Q(owner_id__in=receivers) & Q(owner__friend_receiver_user__state=1))) |
                                          ( Q(view=2) & (Q(owner_id__in=receivers) & Q(owner__friend_receiver_user__state__gte=1)))))).distinct()
                for word in words:
                    viewable_items = viewable_items.filter(Q(artist__contains=word) | Q(title__contains=word)).distinct()
                    if genre != '':
...
class ItemView(View):
    ...
    def post(self, request, item_id):
        ...
        if request.POST['submit_type'] == "save_comment":
            ...
            comment_requests = WatchRequest.objects.filter(Q(item=item) & Q(watch_method=0)).distinct()
            ...
            if request.POST['state_names'] == 'borrow':
                requests = WatchRequest.objects.filter(Q(item=item) & Q(watch_method=1) ).distinct()
            ...
            if request.POST['state_names'] == 'comment':
                requests = WatchRequest.objects.filter(Q(item=item) & Q(watch_method=0)).distinct()
...
def see_announcement(request):
    for ann_id in request.GET.getlist("announcement_ids[]"):
...
def see_notification(request):
    for notification_id in request.GET.getlist('notification_ids[]'):
